...
type Aoe = CircleAoe | RectAoe | FanAoe | DonutAoe;
...
const defaultAssets: LibraryAsset[] = [
  ...
  { id: "default-players-caster", name: "caster", category: "Players", href: "assets/Players/caster.png", tags: inferTags("caster"), source: "default" },
  ...
  { id: "default-players-free", name: "free", category: "Players", href: "assets/Players/free.png", tags: inferTags("free"), source: "default" },
  ...
  { id: "default-players-healer", name: "healer", category: "Players", href: "assets/Players/healer.png", tags: inferTags("healer"), source: "default" },
  ...
  { id: "default-players-melee", name: "melee", category: "Players", href: "assets/Players/melee.png", tags: inferTags("melee"), source: "default" },
  ...
  { id: "default-players-ranged", name: "ranged", category: "Players", href: "assets/Players/ranged.png", tags: inferTags("ranged"), source: "default" },
  ...
  { id: "default-players-tank", name: "tank", category: "Players", href: "assets/Players/tank.png", tags: inferTags("tank"), source: "default" },
...
async function readRasterUrl(url: string): Promise<{ href: string; width: number; height: number }> {
  ...
  try {
    ...
    throw new Error("Arena Imageの読み込みに失敗しました。CORSまたはネットワーク制限の可能性があります。");
  }
  if (!response.ok) throw new Error(`Arena Imageの読み込みに失敗しました。HTTP ${response.status}`);
...
function App() {
  ...
  const [aoeDraft, setAoeDraft] = useState({ label: "AoE", x: 0, y: 0, radiusM: 5, outerRadiusM: 8, innerRadiusM: 3, widthM: 4, heightM: 10, rotation: 0, angleDeg: 90, directionDeg: 90, color: "#dc8030", opacity: 0.32 });
  ...
  useEffect(() => {
    let active = true;
    setArenaImageStatus("Loading Arena Image list...");
    void loadArenaImagesFromGitHub()
      ...
      .catch((error) => {
        ...
        setArenaImageStatus(error instanceof Error ? `${error.message}. Manual fallback list is available.` : "Arena Image list failed. Manual fallback list is available.");
      });
    return () => { active = false; };
  }, []);
  useEffect(() => {
  ...
  const loadArenaImage = async (url: string) => {
    ...
    try {
      ...
      setArenaImageStatus(error instanceof Error ? error.message : "Arena Imageの読み込みに失敗しました。");
    }
  };
  ...
  const importJson = async (file: File) => {
    try {
      if (await hasAutoSave() && !(await requestConfirm({ title: "Load JSON", message: "JSONを読み込むと、現在の自動保存データは読み込んだ内容で上書きされます。続行しますか？", confirmLabel: "Load", cancelLabel: "Cancel" }))) return;
    } catch {
  ...
  const resetToBlankProject = async () => {
    if (!(await requestConfirm({ title: "Reset Blank", message: "現在の編集内容と自動保存を破棄して、初期状態に戻しますか？", confirmLabel: "Reset", cancelLabel: "Cancel" }))) return;
    try {
      await clearAutoSave();
      setUndoStack((s) => [...s.slice(-49), diagram]);
      setRedoStack([]);
