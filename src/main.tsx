  ...
  useEffect(() => {
    ...
    void loadArenaImagesFromGitHub()
      .then((images) => {
        ...
        setArenaImageStatus(images.length > arenaImages.length ? "" : "Arena Images: remote list returned no extra images.");
      })
      .catch((error) => {
        if (!active) return;
        setAvailableArenaImages(arenaImages);
        setArenaImageStatus(error instanceof Error ? `Arena Images list failed: ${error.message}` : "Arena Images list failed.");
      });
    return () => { active = false; };
  }, []);
  useEffect(() => {
